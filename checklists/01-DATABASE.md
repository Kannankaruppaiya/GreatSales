...
| # | Item | Status | Evidence |
| --- | --- | --- | --- |
| A.3.1 | 🔴 **The runtime DB role is NOT superuser and does NOT have `BYPASSRLS`.** Proven by `SELECT rolsuper, rolbypassrls FROM pg_roles WHERE rolname='greatsales_app';` returning `f, f` **in the production database** | `[~]` | `PrismaService.onModuleInit` now checks BOTH `is_superuser` AND `pg_roles.rolbypassrls` and refuses to start on either (extended 2026-08-25 — it previously checked superuser only). Local proof: `pnpm --filter api test` logs `Connected as greatsales_app (not superuser, NOBYPASSRLS)`. **Remaining: run the same query against the PRODUCTION database.** |
| A.3.2 | RLS is `ENABLE`d **and** `FORCE`d on every tenant-owned table (`FORCE` matters — the table owner ignores plain `ENABLE`) | `[ ]` | |
| A.3.3 | A generated inventory proves **every** tenant-owned table has RLS on — no table added later silently missed the dynamic loop. Run `SELECT relname, relrowsecurity, relforcerowsecurity FROM pg_class …` and diff against the model list | `[ ]` | |
| A.3.4 | Every RLS policy covers all four commands (`SELECT`, `INSERT`, `UPDATE`, `DELETE`) — a `SELECT`-only policy still allows cross-tenant writes | `[ ]` | |
...
| A.3.8 | 🔴 **Defence in depth:** every service-layer query also carries an explicit `where: { tenantId }` filter. RLS is the net, not the plan. Grepped and proven for every repository method | `[ ]` | |
| A.3.9 | Raw SQL / `$queryRaw` / `$executeRaw` call sites inventoried — each one is parameterised and tenant-scoped | `[ ]` | |
| A.3.10 | Cross-tenant integration test exists: tenant A authenticated, attempts read **and** write of tenant B's row for **every** resource, gets denied | `[ ]` | |
| A.3.11 | The platform/super-admin layer (`PlatformUser`, `FeatureFlag`, `TenantFeatureFlag`) has its own explicit policy — it is not accidentally readable by tenant users | `[ ]` | |
| A.3.12 | Seed scripts and the seed guard cannot run against production (guard verified by actually pointing it at a prod-shaped URL and watching it refuse) | `[ ]` | |
