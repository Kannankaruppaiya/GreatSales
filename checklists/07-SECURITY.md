...
| G.3.9 | 🔴 No real credential, seed password, or tenant data in any committed file, example env, fixture, or test | `[~]` | PARTIAL. The two seed JSONs are out of git history and gitignored (verified against the object store). `apps/web/src/data/pocSeedData.ts` still carries the same 417 customers and **still ships in the production build** — measured, 50 of 60 sampled literals present. Progress 2026-08-25: it is no longer in the ENTRY chunk (790 kB -> 358 kB) so it is not served before login, and four dead modules that reached it were deleted. **The last path is the F14 management feature, which reads a client-side mock store.** Closing this means F14 stops reading `trackerStore` — or the mock is reseeded with synthetic data, which is a product call since it changes what that screen shows. |
