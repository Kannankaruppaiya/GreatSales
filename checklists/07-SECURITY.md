...
| G.3.9 | 🔴 No real credential, seed password, or tenant data in any committed file, example env, fixture, or test | `[~]` | PARTIAL 2026-08-25. **Removed:** `packages/db/prisma/promech-data.json` and `poc-v6-data.json` (417 customers, 141 payments, and 10 named contacts WITH mobile numbers) were committed, then stripped from branch history while it was still unpushed, gitignored, and fronted by `prisma/dataset.ts`. Verified: no commit references either path, the pre-rewrite commit is unreachable, and no blob of that size survives `gc --prune=now`. **STILL OPEN:** `apps/web/src/data/pocSeedData.ts` (727 KB, tracked, pre-existing) holds the SAME dataset — 417 real customer names, 234 products, 883 mappings, 141 payments — without the contact names or mobile numbers. It cannot simply be deleted: `trackerStore` reads it and the mapping modal and management feature still run on that store. Closing this is the same job as [D.1.6a/D.1.6b](04-FRONTEND-WEB.md), and it needs [C.3.1](03-API.md) (mapping API) and [C.3.5](03-API.md) (real tenancy) first. |
