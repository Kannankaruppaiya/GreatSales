...
| G.3.9 | 🔴 No real credential, seed password, or tenant data in any committed file, example env, fixture, or test | `[x]` | DONE 2026-08-25, in two halves. (1) `promech-data.json` and `poc-v6-data.json` were stripped from git history while the branch was unpushed and gitignored — verified no commit references either path and no blob of that size survives `gc --prune=now`. (2) `apps/web/src/data/pocSeedData.ts` held the same 417 customers and SHIPPED in the production bundle (measured: 50 of 60 sampled literals, in the pre-login entry chunk). It is now deleted and `trackerStore` reads synthetic fixtures. Final check: all 1,746 real data literals probed against `dist/` — 4 hits, every one an app string that coincides (a date, a transporter option, two demo emails), zero customer records. A CI guard blocks reintroduction (D.1.6f). |
