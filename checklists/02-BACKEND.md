...
| B.1.13 | 🔴 API TypeScript runs in **full `strict` mode**. Today `apps/api/tsconfig.json` sets `noImplicitAny: false`, `strictBindCallApply: false`, `noFallthroughCasesInSwitch: false` and never enables `strict` — so implicit `any` flows silently through the entire backend. Turn `strict` on and fix the fallout | `[x]` | DONE 2026-08-25. `strict: true` plus `noImplicitAny`, `strictBindCallApply`, `noFallthroughCasesInSwitch`. Fallout was **2 errors, one root cause** — far smaller than expected. Both were `teams.service.ts` `assertManagerUsable(db: { user: { findFirst: (a: unknown) => Promise<unknown> } })`: a hand-rolled shape that only type-checked because non-strict skipped the parameter check. `unknown` is not assignable to Prisma args, so under strict the REAL client stopped fitting the signature written to describe it. Replaced with the exported `TenantPrisma`. Verified: lint 0/0, types clean, 26 suites / 337 tests, 5 e2e / 135 tests. |
| B.1.14 | `skipLibCheck` and any other loosened compiler flag are reviewed and justified, not inherited from a scaffold | `[~]` | `skipLibCheck: true` remains — it skips type-checking of .d.ts files, which is standard for a Prisma/Nest project (generated Prisma types are large and third-party .d.ts errors are not actionable here). Kept deliberately; not yet stress-tested by turning it off to see what surfaces. |
