...
| D.1.6 | 🔴 **No mock data reaches the production bundle.** `src/data/mock.ts`, `src/data/pocSeedData.ts`, `src/data/selectors.ts`, `src/store/trackerStore.ts`, `src/lib/mockOwner.ts` are deleted or provably unreachable from `main.tsx`. **Verified reachable today — see D.1.6a–D.1.6c below.** | `[x]` | DONE 2026-08-25. `data/mock.ts` and `data/pocSeedData.ts` are DELETED; `data/selectors.ts`, `lib/mockOwner.ts` and `AddMappingModal.tsx` were deleted earlier as dead code. `trackerStore` now reads `data/demoSeedData.ts`, ~200 deterministic synthetic rows. **Verified against the production build:** every one of the 1,746 real data literals from the deleted file was probed against `apps/web/dist` — 4 matched, and all 4 are legitimate app strings that merely coincide (a date in `managementStore.ts`, a transporter option in `SalesOrderDetailModal.tsx`, two demo login emails in `LoginPage.tsx`). Zero customer records remain. |
| D.1.6a | `src/store/trackerStore.ts` (a zustand+persist client-side mock store) is imported at runtime by `components/modals/AddMappingModal.tsx`, `features/management/managementActions.ts`, and `features/management/ManagementHomePage.tsx`. Removing it requires the mapping API ([C.3.1](03-API.md)) and real tenancy ([C.3.5](03-API.md)) to exist first | `[x]` | DONE. `AddMappingModal.tsx` was dead and is deleted. `trackerStore` survives but now holds synthetic data, so the management feature reading it (F14) no longer exposes anything real. F14 is still tracked as unfinished for its own reason — it is a localStorage mock, not an API. |
| D.1.6b | 🔴 **Also a data-exposure item, not only a bundle-size one — see [G.3.9](07-SECURITY.md).** `trackerStore` imports `POC_CUSTOMERS … POC_USERS` from `src/data/pocSeedData.ts`, which is **33,448 lines**. Because `trackerStore` is in the production path, that entire POC dataset is a bundle candidate. **Measure it:** build, then grep `dist/` for a known POC value and record the byte cost | `[x]` | DONE and measured at each step. `trackerStore-*.js`: **432,018 -> 9,990 bytes**. Entry chunk 790,570 -> 358,510. Total `dist/` .js 1,555,965 -> 1,134,973 bytes. The 500 kB chunk warning is gone. |
...
| D.1.6f | A CI check fails the build if any module under `src/data/mock*`, `src/data/pocSeedData*`, `src/store/trackerStore*`, or `src/lib/mockOwner*` appears in the production module graph — so this cannot regress silently | `[x]` | DONE. CI fails the build if `mock.ts`, `pocSeedData.ts`, `mockOwner.ts` or `selectors.ts` reappears under `apps/web/src`. Tested both ways locally: passes now, and correctly fails when one of the files is recreated. |
