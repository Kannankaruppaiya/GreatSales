...
| --- | --- | --- | --- |
| J.1.1 | `pnpm check-types` green across every workspace | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174: api, web AND mobile `check-types` all green on a clean install. Mobile had never run on a developer machine (no local typescript binary) — the code was fine, the local install was not. |
| J.1.2 | `pnpm lint` green with zero warnings tolerated | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174: `pnpm --filter api lint` green with zero errors and zero warnings, on a clean checkout. Scope is api only — web declares no lint script, mobile has `expo lint` which CI does not run. |
| J.1.3 | `pnpm --filter api test` green | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174: 26 suites / 332 tests green against a real postgres:16 service as the RLS-bound role. |
| J.1.4 | `pnpm --filter api test:e2e` green **against a real Postgres with the RLS-bound role** | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174: 5 e2e suites / 133 tests green against a real postgres:16 service, migrated with `migrate deploy`, connected as greatsales_app. |
| J.1.5 | `pnpm --filter web test` green | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174: web tests green on a clean clone. Note 8 assertions in LoginPage.demoEmails.test.ts SKIP there — they read the uncommitted POC dataset; see the note in that file. |
| J.1.6 | `pnpm build` (all workspaces, production mode) green from a clean clone with a cold cache | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174: `pnpm build` green from a fresh checkout with no turbo cache. |
| J.1.7 | Mobile `check-types` green | `[x]` | VERIFIED in CI https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174. It fails only on this developer machine, where apps/mobile/node_modules has no typescript binary — a local install problem, not a code problem. |
| J.1.8 | All of the above run in **CI on every PR** and block merge — **CI does not exist yet; this is a blocker** | `[~]` | Runs on every pull request and is green (https://github.com/Kannankaruppaiya/GreatSales/actions/runs/32856078174). **Merge is NOT blocked yet** — that is a GitHub branch-protection setting on `main`, which no file in this repo can set. Stays unverified until protection is on and a failing run is proven to block. |
