...
| F14 | 🟡 **Backend built; web still on the mock.** The real platform (owner) API now exists (D9 resolved as token-exchange): `POST /platform/auth/login` + `GET /platform/auth/me` (PlatformUser, separate auth), `GET /platform/managements` (cross-tenant list via the quarantined owner connection), `POST /platform/managements` (provisions a real Tenant + system roles + first admin), and `POST /platform/managements/:id/assume` (audited token-exchange that opens a management as its own admin). 15 API integration tests, all against real Postgres. **Still `[ ]` on web:** `features/management/managementActions.ts` remains a localStorage dataset swap over the mock `trackerStore`; the rewire onto these endpoints is the next step. Until that lands the switcher is still a lie to the user, so web stays `[ ]`. |
