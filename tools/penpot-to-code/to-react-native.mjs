...
class Board {
  ...
  /** `part` set: a scalable, decorative component cut from a board (see parts.json). */
  source(componentName, part = null) {
    const jsx = this.node(this.dump.root, null, part ? 4 : 2);
    ...
    const consts = [...this.svgs.entries()]
      ...
      .join("\n");
    if (part) {
      return `/* GENERATED by tools/penpot-to-code/to-react-native.mjs: layers
 * ${part.layers.join(", ")} of the Penpot board ${JSON.stringify(part.boardName)}.
...
export default function ${componentName}({ width = size.width }: { width?: number }) {
...
// Parts: named layers cut out of a board into a component real screens use.
const partsFile = path.join(path.dirname(fileURLToPath(import.meta.url)), "parts.json");
const partsDir = path.resolve(flag("parts-out", "apps/mobilev2/src/components/penpot-parts"));
const parts = fs.existsSync(partsFile) ? JSON.parse(fs.readFileSync(partsFile, "utf8")) : [];
if (parts.length) fs.mkdirSync(partsDir, { recursive: true });
const matches = (name, pattern) =>
  pattern.endsWith("*") ? name.startsWith(pattern.slice(0, -1)) : name === pattern;
for (const part of parts) {
  const dump = JSON.parse(fs.readFileSync(path.join(dumpDir, `${part.board}.json`), "utf8"));
  const picked = dump.root.c.filter((c) => part.layers.some((p) => matches(c.n, p)));
  const missing = part.layers.filter((p) => !dump.root.c.some((c) => matches(c.n, p)));
  if (missing.length) {
    console.error(`parts.json: ${part.component} — no layer ${missing.join(", ")} on ${part.board}`);
    process.exit(1);
  }
  const x0 = Math.min(...picked.map((c) => c.b[0]));
  const y0 = Math.min(...picked.map((c) => c.b[1]));
  const x1 = Math.max(...picked.map((c) => c.b[0] + c.b[2]));
  const y1 = Math.max(...picked.map((c) => c.b[1] + c.b[3]));
  const partRoot = { n: part.component, t: "G", b: [x0, y0, x1 - x0, y1 - y0], c: picked };
  const board = new Board({ ...dump, root: partRoot }, part.component);
  const src = board.source(part.component, { ...part, boardName: dump.root.n });
  fs.writeFileSync(path.join(partsDir, `${part.component}.tsx`), src);
  board.images.forEach((i) => images.add(i));
}
if (parts.length) console.log(`${parts.length} parts -> ${path.relative(root, partsDir)}`);
