 ...
 *
 * Ported verbatim from apps/mobile/src/gs/api.ts. It is not rewritten here on
 * purpose: single-flight refresh, the SecureStore migration path and the web
 * fallback are each the answer to a bug that was already found once, and
 * reaching the same answers a second time by writing a fresh client is the
 * expensive way to get the same file. Fixes belong in both copies until v1
 * is retired.
 *
